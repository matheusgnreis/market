      ...
      $('#btn-confirmation').on('click', function (e) {
        $.getJSON('/v1/applications/' + itemId)
          .done(function (application) {
            ...
            if (!(data.modules = application.module) || isEmpty(application.module)) {
              delete data.modules
            }
            if (!(data.admin_settings = application.json_body) || isEmpty(application.json_body)) {
              delete data.admin_settings
            }
            if (!(data.load_events = application.load_events) || isEmpty(application.load_events)) {
              delete data.load_events
            }
            if (!(data.script_uri = application.script_uri) || isEmpty(application.script_uri)) {
              delete data.script_uri
            }
            if (!(data.github = application.github) || isEmpty(application.github)) {
              delete data.github
            }
            data.status = 'active'
            ...
            if (!(data.auth_scope = application.auth_scope) || isEmpty(application.auth_scope)) {
